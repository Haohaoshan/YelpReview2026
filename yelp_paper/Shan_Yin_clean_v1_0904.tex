...
This study aligns most closely with helpfulness-prediction and engagement-modeling research, but differs by using interpretable linguistic features to identify the textual characteristics associated with Yelp review engagement. Because Yelp does not provide deception labels, the focus remains on linguistic signals of perceived reliability rather than on detecting fraudulent content.
...
Table 11 summarizes these rankings: rank values that fall within the top tier for either model are highlighted in blue, along with their mean percentage differences from Section 5.1.
...
\centering
\includegraphics[width=0.8\textwidth]{fig_lr_shap_rank_distance_colored.png}
\end{figure}
...
\centering
\includegraphics[width=0.80\textwidth]{05_xgb_top_features_shap_dependence.png} 
\end{figure}

\begin{figure}[H]
\caption{Logistic Regression XBETA Plot}    
\centering
\includegraphics[width=0.80\textwidth]{05_lr_top_features_xbeta.png}
\end{figure}
...
\centering
\includegraphics[width=0.80\textwidth]{feature_rank_overall_vs_groups_2x2_overlay.png}
\end{figure}
...
\subsection*{A.1 Data Availability}

The analysis uses the publicly available Yelp Open Dataset. No proprietory or restricted data were used. 

\subsection*{A.2 Reproducibility Details}
...
\begin{enumerate}
\item Kim, S., Pantel, P., Chklovski, T., \& Pennacchiotti, M. (2006). Automatically assessing review helpfulness. \emph{EMNLP}.
\item Jindal, N., \& Liu, B. (2008). Opinion spam and analysis. \emph{WSDM}.
\item Mihalcea, R., \& Strapparava, C. (2009). The lie detector: Explorations in the automatic recognition of deceptive language. \emph{ACL-IJCNLP}.
\item Ghose, A., \& Ipeirotis, P. (2011). Estimating the helpfulness and economic impact of product reviews. \emph{IEEE TKDE}.
\item Ott, M., Choi, Y., Cardie, C., \& Hancock, J. T. (2011). Finding deceptive opinion spam. \emph{ACL}.
\item Feng, S., Banerjee, R., \& Choi, Y. (2012). Syntactic stylometry for deception detection. \emph{ACL}.
\item Mukherjee, A., Venkataraman, V., Liu, B., \& Glance, N. (2013). What Yelp fake review filter might be doing? \emph{ICWSM}.
\item Lundberg, S. M., \& Lee, S.-I. (2017). A unified approach to interpreting model predictions. \emph{NeurIPS}.
\item Qu, X., Li, X., Farkas, C., \& Rose, J. (2020). An attention model of customer expectation to improve review helpfulness prediction. \emph{ECIR}.
\item Du, J., Rong, J., Wang, H., \& Zhang, Y. (2021). Neighbor-aware review helpfulness prediction. \emph{Decision Support Systems}.
\item Saumya, S., \& Kumar, P. (2023). Review helpfulness prediction on e-commerce websites: A comprehensive survey. \emph{Engineering Applications of Artificial Intelligence}.
\end{enumerate}
