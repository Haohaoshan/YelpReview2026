...
To explore this question, we analyze Yelp restaurant reviews using a structured set of linguistic features and two machine-learning models. Engagement votes serve as a clean, review-level proxy for perceived helpfulness. The goal is not only to build predictive models but also to identify the linguistic patterns that characterize high-engagement reviews. Together, these analyses show how linguistic signals can reveal meaningful differences in how readers perceive and respond to online content.

In this study, machine‑learning models are used only as tools to confirm which linguistic features matter most. The goal is not to compare models, but to understand how writing style influences whether a review receives engagement.
...
To evaluate whether linguistic features can predict crowd‑sourced engagement, two supervised learning models are employed. Although both models use the same feature set, they differ in how the input features must be preprocessed due to fundamental differences in their learning mechanisms.
...
The models are trained using an 80/20 train–test split, which is appropriate given the large sample size of more than 260,000 reviews. With a dataset of this scale, a single random split provides stable estimates of out‑of‑sample performance without requiring cross‑validation, while keeping computation manageable during feature engineering and model training. The random split ensures that both sets preserve the natural variation in writing styles and engagement patterns, allowing the evaluation to reflect generalization to unseen reviews.
...
This study directly addresses the research question of which linguistic features most strongly shape engagement in Yelp restaurant reviews. Across descriptive comparisons, temporal segmentation, and model‑based analyses, features such as lexical richness, punctuation use, review length, and syntactic complexity consistently distinguish high‑engagement reviews from low‑engagement ones. The two modeling approaches highlight the same core signals, with Logistic Regression imposing strictly monotonic relationships and XGBoost capturing mild nonlinearities, thresholds, and plateau effects. These modeling differences help clarify how the same underlying linguistic features exert their influence, but the central finding remains the stability of the feature set itself.

For features with largely monotonic behavior—unique lemmas, punctuation count, syntactic depth—both models produce similar directional patterns, indicating stable and interpretable effects. For features with more complex or threshold‑based structure, particularly character count, the linear model can oversimplify or distort the true marginal effect, whereas XGBoost adapts to these nonlinearities and provides a more faithful representation of how linguistic attributes shape engagement. Together, these patterns show that the identity of the influential features is robust across modeling choices, even if the functional form of their effects differs.

The temporal segmentation analysis reinforces this stability. Feature‑importance rankings for reviews written before 2019 (Groups 1 and 2) align closely with the overall ranking, suggesting that once reviews have had sufficient time to accumulate votes, the relationship between linguistic cues and perceived helpfulness is highly consistent. More recent reviews (Group 3) exhibit greater variability, reflecting temporal exposure differences rather than substantive changes in how linguistic features influence engagement. This highlights a data‑collection consideration: because engagement counts accumulate over time, snapshots taken at a single point can introduce systematic bias, especially for newer reviews. Future work should control for this exposure‑time bias—for example, by fixing a uniform engagement‑measurement window or by excluding reviews that have not yet had sufficient time to accumulate validation.

Together, these results show that detailed, varied, and structurally coherent writing tends to resonate more strongly with readers, and that the influential linguistic features identified in this study provide a stable basis for understanding perceived usefulness in user‑generated content. Beyond their theoretical value, the findings also carry immediate practical implications: reviewers can use these signals to craft clearer and more informative content, and the Yelp platform can incorporate them into writing‑support tools or AI‑assisted prompts that encourage high‑quality reviews.
